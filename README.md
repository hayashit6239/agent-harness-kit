...
# 2. 対象リポジトリで
/harness-init
# 3. 開発を進め、PR を作ったら、以下のどちらかを選ぶ (台帳ごとに択一・併用しない):
#    a. 手動モード: 別セッションで
/harness-review-pr
#    b. orchestrator モード: developer / pr reviewer への配車を自動化する (任意・Phase 1 P13)
/harness-orchestrate
```

`/harness-review-pr` (手動モード) と `/harness-orchestrate` (orchestrator モード) は台帳への書込主体になり得るため、同じ台帳に対して両方同時に仕掛けない (`.harness/CLAUDE.harness.md` の単一書込主体の原則)。両方動かすと書込主体が二重化し、台帳の drift を招く。
