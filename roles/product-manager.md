...
- **取得**: open issue 本文の依存参照を抽出し、参照先の `gh issue view <N> --json state` で状態照合する。
...
- **fan-out は `general-purpose`**(`fork` は呼出元文脈を継承し狭い directive を無視して最上位タスクを再実行する)。文脈は各 subagent へ自己完結で渡す。
- **`SendMessage` を使わない**(宛先解決に失敗して結果が消失する)。結果は最終メッセージで直接返す。
- **`gh auth switch` を実行しない**(active アカウントを変えると他セッションの `gh` が壊れる)。
- **台帳(`.harness/plan-progress.json`)に触れない**(Read も編集も。単一 writer は orchestrator)。
- **観測していないことを書かない**(「エラー」と「処理中」を区別・裏取りできない兆候は「未確認」で起票しない)。
