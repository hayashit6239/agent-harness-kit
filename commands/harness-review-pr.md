...
4. **各 PR をレビューする(review-mode で分岐)** — CWD のチェックアウトに対して動くため、wrapper は事前に **per-PR git worktree を作成**して隔離する(user の作業ツリーを汚さない):
   ...
   ```
   - **severity の分類(キーワードベース、実機再検証で得た文言差に基づく)**: `summary` と `failure_scenario` を連結した文字列を小文字化し、次のいずれかを部分一致で含めば `severity = "🔴"`、含まなければ `severity = "🟡"` とする(判断が難しいものは過剰ブロックより見逃しの実害が小さいため 🟡 側に倒す):
     ```
     crash, null, none, undefined, nullpointer, typeerror, "type error", type-unsafe, "type safety",
     "off-by-one", "off by one", race, concurren, deadlock, overflow, underflow,
     クラッシュ, ヌル, 型安全, 並行, 排他, 境界値, 競合状態, デッドロック
     ```
     実機確認: 意図的に導入した correctness bug(`sources` 欠損時に `None` を返すガード除去 → `TypeError`)の finding には `"crash"` `"TypeError"` を含む summary/failure_scenario が返った。一方 cleanup のみの diff(既存の `normalize()` ヘルパーを再利用せず 3 箇所で inline 複製)の finding には `"duplicated"` `"dead/unused code"` `"maintain"` `"inconsistent"` を含む文言が返り、上記キーワードは一切含まれなかった — 両者は文言上明確に分離できる。
...
5.6. **停止条件の判定(round_flag / trend_flag / escalate)** — **手動モード(人間が本コマンドを直接叩く運用)でも常に実行する**(マーカー埋め込み・🛑 セクション表示は無条件。ラベル付与や needs-human エスカレーションの**実行**は行わない — それは `commands/harness-orchestrate.md` 側だけの責務):
   ...
   - **round >= 3 の場合**: 直近 2 件の過去 `# PR Reviewer` コメント本文末尾のマーカー(`<!-- harness-review-pr: round=<N> has_blocker=<true|false> blocker_count=<M> ... -->`)を正規表現でパースし、`blocker_count(N-1)`(直近 1 件前)・`blocker_count(N-2)`(直近 2 件前)を取得する。マーカーが無い(旧フォーマットの過去コメント・件数不足)場合はその過去分を「不明」として trend_flag の判定を成立させない(fail-closed で escalate させない側に倒す — 誤 escalate の方が実害が大きいため)
     ...
     - **`ESCALATE_REASON` の代入**: `escalate == true` の場合のみ、成立した条件に応じて理由文字列を組み立てる(`escalate == false` の場合は空文字列でよい。手順 5 は `escalate == true` のときしかこの変数を参照しない):
       - `round_flag` のみ成立: `"round 上限到達(round ${ROUND})"`
       - `trend_flag` のみ成立: `"blocker 件数が改善していない(round $((ROUND-2)) → $((ROUND-1)) → ${ROUND}: ${BLOCKER_COUNT_N_MINUS_2} → ${BLOCKER_COUNT_N_MINUS_1} → ${BLOCKER_COUNT} の推移)"`
       - 両方成立: 上記 2 つの文字列を読点等で連結する(例: `"round 上限到達(round ${ROUND})。blocker 件数が改善していない(round ... の推移)"`)
   - この手順の結果(`ROUND` / `has_blocker` / `blocker_count` / `escalate` / `ESCALATE_REASON`)を手順 5 の投稿本文に反映する
