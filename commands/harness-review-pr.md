...
- **既定 `review-mode=code-review`**: 8 角度 finder(`commands/harness-dispatch-review-finders.md`)が機械的に収集した候補を、本 wrapper(pr reviewer)自身が**独立検証**(意味的な severity 判定・採否)する(issue #49・**個人 skill 不要**で動く可搬構成)。候補収集の実行主体は起動経路で変わる — 詳細は手順 4-b 参照。
...
4. **各 PR をレビューする(review-mode で分岐)**:
   ...
   **候補収集(finder)は reviewer 自身が担わない**。誰が収集するかは起動経路で決まるが、**どちらの経路でも finder は「収集を実行する主体」の直接の子であり、reviewer の子(≒ orchestrator から見た孫)にはならない**:
   ...
   いずれの経路でも `{file, line, summary, failure_scenario}` の JSON 配列(0 件なら `[]`)を得る(`severity` フィールドは無い。8 角度 finder の収集は correctness に加えて reuse/simplification/efficiency 等の cleanup 角度も含むため、cleanup 系の finding が混在しうる)。**ここから先(severity 付与・採否判断)が独立検証であり、finder には行わせず reviewer 自身が行う**(doer ≠ judge — 収集は機械的、判定は reviewer):
   - **severity の分類(意味判断・独立検証)**: 各 finding について、wrapper を実行している Claude 自身が `summary` + `failure_scenario` を読み、**意味的に** severity を付与する(キーワード表には依らない — 表に無い語彙の真正 correctness bug を 🟡 に落として素通しさせる過補正を避けるため):
