...
- `jq`: pr reviewer ロール (`roles/pr-reviewer.md`。orchestrator が dispatch する) の台帳選別に必要。
...
| `.harness/roadmap.json` | `roadmap.json` を**そのまま複製** (真に空の `{"phases": []}` 雛形)。phase → epic 対応の canonical source (`rules/issue-tree.md` §6)。導入先が初回に自 repo の phase を追記する |
...
`.harness/roadmap.json` は phase → epic 番号の対応表 (`rules/issue-tree.md` §6 が定義する canonical source) で、**中身が導入先ごとに分岐するインスタンスデータ**。seed は真に空の `{"phases": []}` で、導入先が自 repo の phase ラベルと epic issue の対応を `{"phase": "<ラベル名>", "epic": <番号 or null>, "title": "<人間可読名>"}` の形で追記する (`plan-progress.json` と同じく git 追跡対象・byte 一致検査対象外)。空 / 未設定でも消費者は fail-soft で 1 行 surface するだけで壊れない。
...
- `gh label create "ready for implementation" --color "1d76db" --description "issue reviewer が実装着手可能と判定した issue" --force` を実行する (`--force` で冪等。issue reviewer ロールが使うラベル。**人への作業依頼にしない** — このコマンドがここで作る。色・説明は `commands/harness-orchestrate.md`「label_action の実行」節の同ラベル create fallback と**必ず揃える** — `--force` が既存定義を上書きするため片側だけ変えると drift する。`ready for merge` の PR 版と対称・issue #88)。
- `gh label create "discover" --color "5319e7" --description "discover→enqueue: 台帳へ自動 enqueue する対象の open issue (issue #78)" --force` を実行する (`--force` で冪等。**人間が opt-in で issue に付ける入力ラベル**。orchestrator の「discover→enqueue フェーズ」(`commands/harness-orchestrate.md`) がこのラベル付き open issue を発見して台帳へ step 化する・issue #78 能力3。**人への作業依頼にしない** — 導入先 repo での移植性のためここで作る。`ready for merge` / `ready for implementation` と違い orchestrator は付与/除去しない入力ラベルなので「label_action の実行」節に create fallback は持たせない)。
- 前提 skill の存在を検査する: `reviewing-multi-angle` / `reviewing-pr-architecture` / `reviewing-pr-google-method` は `~/.claude/skills/<name>/SKILL.md` を `test -f` で確認し、`/code-review` は利用可能な skill 一覧にあることを確認する。無いものがあれば**警告**を出す (init 自体は続行してよいが、pr reviewer ロール (`roles/pr-reviewer.md`) の review-mode=multi-angle には必須と伝える)。
