...
これは **運用(policy)** の層であり、minimal 構成の上に乗る **orchestrator ロール**(Phase 1 の中核機構の最初の増分)。対象は **PR ライフサイクルの 3 ロール(developer 実装役・対応役 / pr reviewer)+ issue フェーズの 2 ロール(issue reviewer / issue review worker)**(issue #88 で issue フェーズを PR フェーズと対称に配線した)。**issue フェーズの非対称(正直な明記・issue #88)**: 「A＝PR と対称」は 2 点を除いて成立する — (i) issue フェーズには実行して落とせる証拠(test)が構造的に無く **evidence gate 相当が無い**、(ii) issue reviewer の判定エンジンは v1 では kit 非同梱の個人 skill `reviewing-github-issues` に依存する **opt-in path で可搬でない**(PR 既定 `code-review` は #49 で可搬化済み)。完全可搬化(8 観点 rubric を `roles/issue-reviewer.md` へ inline)は follow-up。
...
# -> {"groups": [["<id>", ...], ...], "safe": ["<id>", ...]}
```
**代表選出述語(issue #55・round3 で規則X/Y を一本化)**: script が返す `groups` / `safe` を prose 側で次のとおり dispatch へ落とす。**不変条件は「1 つの衝突 group から今 tick に dispatch されるのは常に高々 1 件。live wait 占有者が居る group からは 0 件」**:
...
- **`groups`**(size≥2 の衝突組、および対象ファイル抽出 0 件で fail-closed 判定された単独候補)は group ごとに:
...
**redispatch の扱い(規則Y・round3 で限定)**: redispatch 候補は原則 #26/#71 機構どおり dispatch 経路に残す(占有者述語で**無条件に**止めると retry_count が消費されず締切 sink が殺されるため)。ただし round3 で **「同一ファイルに live wait 占有者を共有しない redispatch に限る」へ限定**した — 同一ファイルに live wait 占有者が居る redispatch は、上記「占有者が居る group からは 0 件」により**占有者が clear(PR 作成)/ sink するまで marker 非書換で持ち越す**。この持ち越しは、上記 group 持ち越し(代表選出で負けた候補の marker 非書換持ち越し)/「配車テーブル」節が定める「実装役枠から溢れた redispatch の marker 非書換持ち越し」が既に定める「持ち越し = marker 非書換で次 tick」操作の再利用で、新機構は足さない。round2 は「redispatch を無条件に止めない」まで振り切り 2 経路(占有者不在で redispatch × 新規 eligible が代表枠を争う経路 A / live wait 占有者 × 同一ファイル redispatch の経路 B)で normal 運用でも 2 件同時 in-flight に到達していたが、上記の限定 + 一本化で両経路とも「高々 1 件」に畳まれ decidable になった。
...
**machine-enforce の限界(DoD (iv)・正直な明記)**: 上記の代表選出・占有者除外・step id 昇順 tie-break はすべて **prose 側の判断**であり、`detect-dispatch-collision.py` は grouping しか行わない。したがって smoke(`tests/smoke/run-smoke.sh` [12])が実検証できるのは「wait 占有者ファイルを新規候補と同一 group にまとめた入力 → size≥2 group / safe=[] になる」**grouping 層まで**で、「占有者が居る group から dispatch 0」「代表 1 件」という prose の判断結果そのものではない。この prose 判断は「既知の制限・拡張ポイント」節の『ファイル衝突検知の抽出規則は machine-enforce されていない』と同型に machine-enforce されず(pure-function 境界を守るため)、headline の多 tick 挙動とあわせて**実運用観測に委ねる**(kit 前例: 抽出規則の machine-enforce 不在 / 台帳「自己申告は便宜シグナル」に倣う)。
...
2. **dispatch**(ツール制限は pr reviewer と同じ。`gh issue comment` 投稿は許可するが台帳・ラベルには触れさせない)。**dispatch prompt 本文は `${CLAUDE_PLUGIN_ROOT}/roles/issue-reviewer-dispatch.md` へ外出し済み**(★最重要★ の共通コア + issue reviewer 固有項目 + skill 起動 → blocker_count/round/prev_markers 補完 → `evaluate-stop-condition.py` で escalate 補完 → marker 埋込 + 投稿 → `contracts/issue-reviewer-return.schema.json` の形で返す指示を含む)。この参照ファイルは **dispatch する subagent 自身が読む** — orchestrator 自身は読まない:
   ...
   > 「`${CLAUDE_PLUGIN_ROOT}/roles/issue-reviewer-dispatch.md` を Read し、そこに書かれた指示をそのまま実行せよ(対象 issue は #<N>)。手順本体は転写しない — 必ずファイルを Read してから実行すること。」
...
- **恒久衝突ペアの代表選出・占有者除外(issue #55)も machine-enforce されていない(既知のギャップ・DoD (iv))**: 「ファイル衝突検知」節の代表選出述語(「1 group から dispatch は高々 1 件・live wait 占有者が居れば 0 件」・step id 昇順 tie-break・wait 占有者の inject / 除外・「同一ファイルに live wait 占有者を共有しない redispatch に限り止めない」)は、上記の抽出規則と同じ理由で prose 側に置く(占有者判定に台帳 state(`dispatchMarker`/`pr.number`)を要し、script へ入れると pure・stdin 完結境界を壊す・🟡4)。`detect-dispatch-collision.py` は Union-Find grouping のみで代表選出も占有者概念も持たない(**issue #55 で script は不変**)。したがって smoke `[12]` が実検証するのは「wait 占有者 / redispatch / 新規 eligible を同一ファイルで同一 group にまとめた入力 → size≥2 group / safe=[]」という grouping 層までで、「代表 1 件」「占有者 group から 0 件」という prose の判断結果は検証しない。headline の「両者が最終的に dispatch されデッドロックしない」(多 tick の temporal property・DoD (i))も per-tick 純関数アサートの合成 + 実運用観測に委ねる(tick-simulator は足さない・over-engineering 回避)。将来 #87 の decision script 抽出パターンが定まれば script 抽出候補とする(#55 は #87 に依存しない)。
