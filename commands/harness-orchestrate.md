...
1. **network discover(orchestrator の I/O・散文・smoke 対象外=手動確認)**: 次を 1 回実行し、opt-in ラベル `discover` が付いた open issue の候補を、**epic 判定材料(epic ラベル / `epic:` prefix)と本文(`body`)を添えた `{number, isEpic, body}` オブジェクト配列**として得る(epic 除外 fail-safe + `Depends-on` 依存変換の入力・issue #107・D1 / issue #111・D4):
   ```
   gh issue list --repo <owner/repo> --label discover --state open --json number,labels,title,body \
     --jq '[.[] | {number, isEpic: ((.labels | map(.name) | index("epic") != null) or (.title | test("^epic(\\(.*\\))?!?:"))), body}]'
   ```
   ...
   - **`body` は `Depends-on: #N` 依存変換の入力(issue #111・D4)**: 本文を script へ渡し、script 側で `Depends-on` marker を parse して依存先 issue 番号を得る(parse・番号→id 解決・fail-closed skip は決定論のため script 側 = smoke 被覆。本文取得(network)だけが prose = 手動確認)。`body` を取れなかった / マーカーが無い候補は「依存なし」= 従来挙動不変。
...
2. **純 enqueue/dedup/epic 除外/依存変換(script・smoke 対象)**: 得た候補オブジェクト配列と現台帳 steps を `scripts/decide-enqueue-steps.py` へ渡し、追加すべき step 群と fail-closed skip した候補群を得る(既存 `reconcile-dispatch-marker.py` / `decide-orchestrator-route.py` と同型の pure decision script・stdin JSON → stdout・network 非依存・決定論):
   ```
   PLAN="$(git rev-parse --show-toplevel)/.harness/plan-progress.json"
   ENQUEUE=$(jq -cn --argjson cand "<候補オブジェクト配列(上記 discover の出力・{number,isEpic,body})>" --slurpfile plan "$PLAN" \
     '{candidates: $cand, steps: $plan[0].steps}' \
     | python3 "${CLAUDE_PLUGIN_ROOT}/scripts/decide-enqueue-steps.py")
   # -> {"enqueue": [<追加 step>, ...], "skipped": [{number, reason}, ...]}   (どちらも空配列 = no-op / skip 無し)
   ```
   入出力契約 = `{候補 (裸 int | {number, isEpic, body, ...}) リスト, 現台帳 steps} → {追加 step 群 / no-op, 依存 fail-closed skip した候補群}`。この script が **epic 除外 fail-safe(`isEpic:true` の候補を dedup/採番の前段で決定論的に drop・id 番号を消費しない・issue #107・D1。epic は最下層の実装単位でなく PR で close できないため、誤って `discover` ラベルが付いても台帳 step 化しない)/ dedup(dedup key=`issue.number` 一致・突合範囲=全 step(終端 `closed issue` / `merged pr` を含む)・終端後の再ラベル=no-op・round1 🔴2)/ batch 採番(同一 tick で複数候補を enqueue する際は max+1, max+2, … と逐次加算・round2 🟡2。固定値 DoD「1 件追加」は単一対象 happy-path として不変)/ **`Depends-on: #N` → 台帳 `dependsOn` 変換(issue #111)**: `body` から `Depends-on` marker を parse(D3・複数 distinct union・コードフェンス/引用除外)し、依存先 issue 番号を「既存 step の `issue.number` → step id」∪「同一 batch の割当予定 id」で解決する 2-pass(D1)。解決は**接地(grounding)の least fixpoint**(D8)で行い、依存先が「既存 step か別の grounded な batch 候補」に解決する候補だけを enqueue する — self-loop / 同一 batch 循環 / 未解決 `#N`(D1-iii)/ 推移 skip(skip された候補への依存元)の 4 ケースを 1 条件で fail-closed skip し、**ghost id(`validate-plan-progress.py` が非存在 step id / 循環で全台帳 reject する `dependsOn`)を構造的に出さない**(D2)。skip した候補は stdout の `skipped:[{number, reason}]` へ出す(D9・exit 0 維持)/ step 雛形生成(`id`=既存台帳の形式に追随した max+1 の string(`P<n>` 台帳なら `P<max+1>`・純数値台帳なら `<max+1>`・issue #78 round1 🔴) / `issue={number:<N>, status:"created issue", githubState:"open"}` / `pr={number:null,...}` / 依存があれば解決済み step id の `dependsOn`・無ければキー省略・round1 🟡1)** をすべて決定論的に担う。空入力(候補 0 件)= no-op(round1 🟡2)。**候補要素は後方互換で裸の `issue.number`(int)も受理する**が、epic 除外を効かせるには `{number, isEpic}` を、依存変換を効かせるには `body` を渡す(裸 int は `isEpic=false` / 依存なし扱い)。**受容コスト(D5)**: dedup key=`issue.number` のため、既に step 化済みの issue へ後から `Depends-on:` を足しても no-op で台帳へ届かない(変換は初回 enqueue 時 1 回きり)。`Depends-on` は当該 issue に `discover` を付ける**前**に書くことで回避できる(再同期経路は follow-up)。**`Depends-on` 書式の暫定唯一の正は #111 parser(`decide-enqueue-steps.py` の `_parse_depends_on` docstring)**(D6)で、authoring 側 doc(`rules/issue-tree.md` §4 / `issue-authoring.md`)への書式追記はマーカーを書く側の規約 = #107/#109 の領分に委譲する。
3. **台帳書込 + skip surface(orchestrator・単一 writer)**: orchestrator が script の返す `enqueue` 配列の各 step を**台帳の `steps` へ append する**(発見役は候補報告のみ・enqueue の台帳書込は orchestrator tick に集約する単一 writer 不変条件・round1 🔴3。discover subagent が直接台帳を書く経路は禁止)。append はローカル台帳の直接編集(F案・`git add`/`commit` はしない)。`enqueue` が空なら書込なし(no-op)。書込後、追加 step は同一 tick の選別(jq)から見える。**さらに `skipped`(依存 fail-closed で落とした候補)が非空なら、`{number, reason}` を tick 報告へ 1 行 relay する(issue #111・D9)** — これで「未解決 `#N` が typo なら人間が気付く」緩和の閉ループが、消える stderr ではなく構造化 stdout で閉じる。skip は恒久ではなく、依存先が後続 tick で enqueue されれば当該候補も次 tick で解決して enqueue される(各 tick で再評価される)。
...
**テスト境界(round2 🟡1 / issue #111・D4/D7)**: network discover(label クエリ + epic 判定材料 + `body` の取得・上記 1.)は smoke 対象外=手動の最小動作確認。純 enqueue/dedup/epic 除外/依存変換 script(上記 2.)は smoke 対象で、`tests/smoke/run-smoke.sh` が既存の pure decision script([7]/[8]/[9])と同型に決定論検証する(dedup / batch 採番 / 空入力 / 終端突合 / step 雛形 / **epic 除外(`isEpic:true` を drop・id 番号を消費しない)/ 拡張 candidate 契約(裸 int | {number,isEpic,body,...} の受理・未知キー無視)/ `Depends-on` 変換(marker parse・複数 distinct union・フェンス引用除外・2-pass 番号→id 解決・接地 fixpoint による self-loop/循環/未解決/推移 skip surface・ghost id 不出力・blast radius 限定)** の境界 + 不正入力 exit 2)。**依存変換が閉ループ(marker parse + 番号→id 解決 + fail-closed skip + `dependsOn` 付与が決定論テストに乗る)**なのは判定を script 側に置いたためで、prose 側(`body` を含む network 取得と `skipped` の human relay)は smoke 対象外に残る(round2 🟢3 / #111・D4 の受容境界)。
