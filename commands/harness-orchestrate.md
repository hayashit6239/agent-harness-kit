...
**machine-enforce の限界(DoD (iv)・正直な明記)**: 上記の代表選出・占有者除外・step id 昇順 tie-break・fail-closed 単独候補の持ち越しはすべて **prose 側の判断**であり、`detect-dispatch-collision.py` は grouping しか行わない。smoke(`[12]`)が実検証するのは grouping 層(同一ファイルを同一 group にまとめる / `files=[]` を単独 group に落とす)までで、「占有者が居る group から dispatch 0」「代表 1 件」「fail-closed は持ち越し」という prose の判断結果そのものは検証しない。**詳細と根拠は「既知の制限・拡張ポイント」節の該当項目(『恒久衝突ペアの代表選出・占有者除外(issue #55)も machine-enforce されていない』)に一本化する**(重複記載を避けるため、ここでは要旨のみ)。
...
- **恒久衝突ペアの代表選出・占有者除外(issue #55)も machine-enforce されていない(既知のギャップ・DoD (iv))**: 「ファイル衝突検知」節の代表選出述語(「1 group から dispatch は高々 1 件・live wait 占有者が居れば 0 件」・step id 昇順 tie-break・wait 占有者の inject / 除外・「同一ファイルに live wait 占有者を共有しない redispatch に限り止めない」・**fail-closed 単独候補(`files=[]`)は代表選出の母集団に含めず常に持ち越す**)は、上記の抽出規則と同じ理由で prose 側に置く(占有者判定に台帳 state(`dispatchMarker`/`pr.number`)を要し、script へ入れると pure・stdin 完結境界を壊す・🟡4)。`detect-dispatch-collision.py` は Union-Find grouping のみで代表選出も占有者概念も持たない(**issue #55 で script は不変**)。したがって smoke `[12]` が実検証するのは「wait 占有者 / redispatch / 新規 eligible を同一ファイルで同一 group にまとめた入力 → size≥2 group / safe=[]」という grouping 層までで、「代表 1 件」「占有者 group から 0 件」という prose の判断結果は検証しない。headline の「両者が最終的に dispatch されデッドロックしない」(多 tick の temporal property・DoD (i))も per-tick 純関数アサートの合成 + 実運用観測に委ねる(tick-simulator は足さない・over-engineering 回避)。将来 #87 の decision script 抽出パターンが定まれば script 抽出候補とする(#55 は #87 に依存しない)。
