...
const ALL_CHARACTERS: readonly CharacterId[] = ['implementer', 'responder', 'pr-reviewer', 'issue-reviewer'];

/**
 * 4 キャラの state を一括アサート (issue #97 で 2 → 4 値化)。expected に無いキャラは idle を要求する。
 * これで「1 体だけ動いて他 3 体は idle」を毎回明示せず簡潔に書ける。
 */
function expectStates(board: BoardState, expected: Partial<Record<CharacterId, CharacterState>>): void {
  for (const id of ALL_CHARACTERS) {
    expect(board.characters[id].state).toBe(expected[id] ?? 'idle');
  }
}
...
describe('対応表 — issue フェーズ単独 (pr.status = null) のキャラ信号 (issue #97 4 ロール化)', () => {
  // ready for implementation は pr=null のとき実装者 working へ昇格する (規則 5・issue #97 🟡1)。
  it.each([
    [null, {}],
    ['created issue', { 'issue-reviewer': 'waiting' }],
    ['starting review', { 'issue-reviewer': 'working' }],
    ['completed review', { responder: 'waiting' }],
    ['ready for implementation', { implementer: 'working' }],
    ['starting review work', { responder: 'working' }],
    ['waiting for review', { 'issue-reviewer': 'waiting' }],
    ['closed issue', {}],
  ] as [string | null, Partial<Record<CharacterId, CharacterState>>][])('issue.status=%s', (status, expected) => {
    const board = derive(ledgerOf([step('S', status, null)]));
    expectStates(board, expected);
...
describe('対応表 — PR フェーズのキャラ信号 (issue #97 4 ロール化)', () => {
  it.each([
    ['implementation-ready', { implementer: 'waiting' }, false, false],
    ['created pr', { 'pr-reviewer': 'waiting' }, false, false],
    ['starting review', { 'pr-reviewer': 'working' }, false, false],
    ['completed review', { responder: 'waiting' }, false, false],
    ['need for human review', {}, false, true],
    ['ready for merge', {}, true, false],
    ['starting review work', { responder: 'working' }, false, false],
    ['waiting for review', { 'pr-reviewer': 'waiting' }, false, false],
    ['merged pr', {}, false, false],
  ] as [string, Partial<Record<CharacterId, CharacterState>>, boolean, boolean][])(
    'pr.status=%s → %o / celebrate=%s / escalate=%s',
    (status, expected, celebrate, escalate) => {
...
describe('規則 5 — 実装者 working 昇格 (issue #97 🟡1 / 🟡C)', () => {
  it('issue=ready for implementation ∧ pr=null → 実装者 working (「実装中」・折り畳み段の特例)', () => {
    const board = derive(ledgerOf([step('S', 'ready for implementation', null)]));
    expectStates(board, { implementer: 'working' });
    // 昇格時の task ラベルは「実装中」(信号表の「実装着手待ち」ではない)
    expect(board.characters.implementer.tasks).toEqual(['S issue: ready for implementation (実装中)']);
  });

  it('pr!=null なら PR 信号が勝ち昇格しない (規則 1 の非侵食 guard)', () => {
    const board = derive(ledgerOf([step('S', 'ready for implementation', 'created pr')]));
    // PR フェーズ開始済み → pr-reviewer waiting のみ。implementer は idle (昇格しない)
    expectStates(board, { 'pr-reviewer': 'waiting' });
  });

  it('PR 終端 (merged pr) でも昇格しない (pr!=null・信号なし)', () => {
    const board = derive(ledgerOf([step('S', 'ready for implementation', 'merged pr')]));
    expectStates(board, {});
  });

  it('列色 (statusOwner) は昇格の影響を受けず信号表由来のまま (developer 系)', () => {
    // 昇格は折り畳み段のキャラ状態のみを変え、列色の由来 (statusOwner = 信号表の character) は不変
    expect(statusOwner('issue', 'ready for implementation')).toBe('implementer');
    expect(colorGroup('implementer')).toBe('developer');
  });
});

describe('合成規則 1 — pr.status != null の step は PR フェーズの信号のみ採用', () => {
  it('issue の working 信号は pr が進行中なら採用されない', () => {
    // issue: starting review work (責 responder working) / pr: created pr (pr-reviewer waiting)
    const board = derive(ledgerOf([step('S', 'starting review work', 'created pr')]));
    expectStates(board, { 'pr-reviewer': 'waiting' }); // responder は idle (issue 信号は不採用)
  });

  it('follow-up 型 step (現台帳 P8 相当): issue 側の残ボールはキャラ信号に出さない (意図的除外)', () => {
    // issue: created issue (issue-reviewer 待ちに見える) / pr: completed review (responder 対応待ち)
    const board = derive(ledgerOf([step('P8', 'created issue', 'completed review')]));
    expectStates(board, { responder: 'waiting' }); // issue-reviewer は idle
  });
...
describe('合成規則 2 — 作業中 > 待ち仕事あり > idle で 1 状態に畳む', () => {
  it('waiting と working が混在したら working が勝つ', () => {
    ...
    expect(board.characters['issue-reviewer'].state).toBe('working');
  });
  ...
  it('キャラごとに独立して畳む', () => {
    ...
    expectStates(board, { responder: 'working', 'pr-reviewer': 'waiting' });
  });

  it('tasks に信号の由来 (stepId + フェーズ + status) が載る', () => {
    const board = derive(ledgerOf([step('P8', null, 'completed review')]));
    expect(board.characters.responder.tasks).toEqual(['P8 pr: completed review (対応待ち)']);
    expect(board.characters['pr-reviewer'].tasks).toEqual([]);
  });
});
...
describe('合成規則 3 — 祝いは舞台全体のフラグでキャラ状態と直交', () => {
  ...
  it('祝いとキャラの作業中は同時に成立する (直交)', () => {
    ...
    expect(board.celebrate).toBe(true);
    expect(board.characters.responder.state).toBe('working');
  });

  it('ready for merge 自体はどのキャラにも信号を出さない (merge は人間の専権)', () => {
    const board = derive(ledgerOf([step('A', null, 'ready for merge')]));
    expectStates(board, {});
    for (const id of ALL_CHARACTERS) expect(board.characters[id].tasks).toEqual([]);
  });
});
...
describe('fail-soft — 未知 status は信号に数えず警告', () => {
  ...
  it('未知の issue status は規則 1 で信号除外される場合も警告される (警告は採否と独立)', () => {
    ...
    expect(board.characters['pr-reviewer'].state).toBe('working'); // pr 側の既知信号は生きる
  });
...
describe('fail-soft — issue/pr オブジェクト欠落 (台帳の手編集) でも描画を壊さない', () => {
  ...
  it('pr オブジェクト欠落: 例外なし + missing-phase 警告 + pr.known=false (issue 信号は生きる)', () => {
    ...
    expect(board.characters['issue-reviewer'].state).toBe('working'); // issue 側の信号は劣化せず採用
    expect(board.celebrate).toBe(false);
  });
  ...
  it('issue オブジェクト欠落: 例外なし + missing-phase 警告 + issue.known=false (pr 信号は生きる)', () => {
    ...
    expect(board.characters['pr-reviewer'].state).toBe('waiting');
  });
  ...
  it('step がオブジェクトですらない場合も落ちない (両フェーズ missing-phase + id は位置で補完)', () => {
    ...
    expect(board.characters['issue-reviewer'].state).toBe('waiting'); // 壊れた step 以外は通常どおり
  });
...
describe('敵対的入力 — status キー欠落 (undefined) は明示 null (未着手) と区別する', () => {
  ...
  it('pr.status キー欠落 → missing-status 警告 + known=false (issue 信号は生きる)', () => {
    ...
    expect(board.steps[0]!.pr.known).toBe(false);
    expect(board.characters['issue-reviewer'].state).toBe('working'); // PR フェーズ未開始扱いで issue 信号は採用
  });
...
describe('敵対的入力 — 重複 step id でも表示キーは一意', () => {
  ...
  it('重複 id でもキャラ信号は両方の step から畳まれる (警告は表示のためで信号は壊さない)', () => {
    ...
    expect(board.characters['issue-reviewer'].state).toBe('working');
    expect(board.characters['issue-reviewer'].tasks).toHaveLength(2);
  });
});

describe('statusOwner — 列ヘッダの色分け (信号表からの導出。issue #97 で 4 値化)', () => {
  it('その status のボールを持つロール (4 値 CharacterId) を返す (信号表と一致)', () => {
    expect(statusOwner('issue', 'created issue')).toBe('issue-reviewer');
    expect(statusOwner('issue', 'starting review work')).toBe('responder');
    expect(statusOwner('pr', 'implementation-ready')).toBe('implementer');
    expect(statusOwner('pr', 'starting review')).toBe('pr-reviewer');
    expect(statusOwner('pr', 'completed review')).toBe('responder');
  });
...
describe('colorGroup — キャラ 4 値 → Kanban 列色 2 系への写像 (issue #97 🟡2)', () => {
  it('実装者・対応者 = developer 系 / PR・Issue レビュー者 = reviewer 系', () => {
    expect(colorGroup('implementer')).toBe('developer');
    expect(colorGroup('responder')).toBe('developer');
    expect(colorGroup('pr-reviewer')).toBe('reviewer');
    expect(colorGroup('issue-reviewer')).toBe('reviewer');
  });

  it('statusOwner → colorGroup の合成が既存 CSS 2 クラス (role-developer / role-reviewer) に載る', () => {
    // KanbanBoard.roleClass が使う写像。4 値 identity を保ちつつ列色は 2 系統に据え置く単一ソース。
    expect(colorGroup(statusOwner('issue', 'created issue')!)).toBe('reviewer');
    expect(colorGroup(statusOwner('pr', 'implementation-ready')!)).toBe('developer');
    expect(colorGroup(statusOwner('pr', 'starting review')!)).toBe('reviewer');
    expect(colorGroup(statusOwner('issue', 'starting review work')!)).toBe('developer');
  });
});

describe('台帳に動きなし = 全キャラ idle', () => {
  it('全 step が null または終端 (closed issue / merged pr) なら全キャラ idle・celebrate/escalate なし', () => {
    const board = derive(
      ledgerOf([
        step('A', null, null),
