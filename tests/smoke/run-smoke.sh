...
#    (fork / SendMessage / gh auth switch / 台帳保護 / 観測禁止)を、7 dispatch ファイル
#    (実装役 / 対応役 / pr reviewer / collectors / issue reviewer / issue review worker・issue #88 /
#    product manager・issue #107)の
...
# 15. decide-enqueue-steps (discover→enqueue の純 enqueue/dedup/epic 除外/依存変換 判定・
#    issue #78/#107/#111) の単体判定が期待通り (候補 + 現台帳 steps → 追加 step / no-op / skip)。
#    単一追加(DoD 1 件追加)/ 冪等 no-op(既存 issue.number 一致 = 二重登録なし)/ batch 採番
#    (max+1, max+2 逐次加算)/ batch 内重複 / 空入力 = no-op / 終端 step 突合(全 step・終端後再ラベル
#    no-op)/ 起点 id / 非数値 id 除外 の境界に加え、issue #107 の epic 除外 fail-safe(isEpic:true を
#    drop・id 番号を消費しない)と拡張 candidate 契約(裸 int | {number, isEpic, body, ...} の受理・
#    未知キー無視)を含む。**issue #111 の `Depends-on: #N` → 台帳 `dependsOn` 変換**: body からの
#    marker parse(D3・複数 distinct union・フェンス/引用除外)/ 番号→step id 解決の 2-pass(D1・
#    既存 step と同一 batch 割当予定 id の統合)/ 接地の least fixpoint による fail-closed skip(D8・
#    self-loop / 同一 batch 循環 / 未解決 / 推移 skip の 4 ケースを 1 条件で閉じ ghost id を構造排除)/
#    stdout `skipped:[{number,reason}]`(D9・exit 0 維持)/ blast radius を skip 候補に限定 / body 欠損
#    後方互換 も固定する。加えて **DoD の end-to-end 結線**: #111 が生成した `dependsOn` を #51 の
#    選別 jq (SELECT_IMPLEMENTER_JQ) へ実際に通し、依存先が未終端の間は eligible にならない / 終端後は
#    eligible になる(負の自己検証)ことを固定する。不正入力 exit 2(number 欠落 / isEpic 非 bool /
#    body 非文字列 を含む)の境界も。network discover(gh issue list --label ... epic 判定材料 + body
#    の取得)は smoke 対象外=手動確認 (round2 🟡1・#111 D4/D7)。
...
# roles/product-manager.md(issue #107)を含める理由: PM ロールは pr/issue reviewer と違い
# 別立ての dispatch ラッパ(*-dispatch.md)を持たず、policy 層の commands/harness-product-manage.md は
# `/loop` 直起動の入口として共通コアを honest downgrade で転写しない(harness-orchestrate.md が
# DISPATCH_FILES 外なのと同扱い)。そのため PM 経路で共通コアを逐語で持つ唯一のファイルが
# roles/product-manager.md であり、その ★最重要★ ブロックを本検査に載せる(fan-out する PM 経路の
# 安全コアを機械検査外に置かないため)。spec が core を持つのは pr/issue reviewer の非対称だが、
# PM には core を持つ別ラッパが無いことが根拠。
DISPATCH_FILES=(
...
# 正: 7 dispatch ファイルすべての冒頭ブロックが 5 canonical 行を含む
...
echo "[13/17] 共通コア presence 検査 OK (7 dispatch ファイル × 5 canonical 行・逐語部分一致・単一ソースは CANONICAL_CORE)"
...
# presence + 負の自己検証)。実測の結果 [13] の CANONICAL_CORE は 7 *dispatch* ファイルの禁止事項
...
# 側が既に保持)ため DISPATCH_FILES(=[13] の共通コア検査対象・7 ファイル)には**加えない**
# (PR 側 roles/pr-reviewer.md が DISPATCH_FILES に無いのと対称。(b)(c) は CANONICAL_CORE とは別軸の検査)。
# 【非対称の但し書き(issue #107)】pr/issue reviewer の spec は core を持たない別ラッパ(*-dispatch.md)が
# core を担うため DISPATCH_FILES 外だが、product manager は別ラッパを持たず policy 層 command が
# honest downgrade で core を転写しないため、spec roles/product-manager.md が core を担い DISPATCH_FILES に
# 入る(= 7 ファイル目)。「spec は常に DISPATCH_FILES 外」ではなく「core を持つ別ラッパがある spec だけが外」。
...
# 両モード雛形の冒頭 + 要件 (b) の手順書参照 literal(issue #110)を lockstep で固定する。
# (b) は生成 goal 文が下流 fresh session でも手順書を辿れるようにする自己参照文言で、両雛形へ焼き込み済み。
assert_present '/goal 「PR #<N> を ready for merge になるまでレビュー・対応を繰り返して。手順は手順書 `commands/harness-orchestrate.md` を正として参照して進めて。'
assert_present '/goal 「issue #<N> を issue レビュー → ready for implementation → PR 作成 → PR レビュー・対応 → ready for merge まで一気通貫で進めて。手順は手順書 `commands/harness-orchestrate.md` を正として参照して進めて。'
...
# --- 16. decide-enqueue-steps (discover→enqueue の純 enqueue/dedup/epic 除外/依存変換 判定・
#     issue #78/#107/#111) ---
...
# フェーズの中間層 (候補 + 現台帳 steps → 追加 step / no-op / skip) を検証する。network discover
# (gh issue list --label ... epic 判定材料 + body の取得) は smoke 対象外=手動確認 (round2 🟡1・#111 D4)。
...
# と **拡張 candidate 契約 (裸 int | {number, isEpic, body, ...} の受理・未知キー無視)** を境界込みで
# 固定する。DoD「1 件追加・二重登録なし」と batch fixture「既存 1 + 新規 2 (1 件 issue.number 重複)
# → 追加 1 件・id=max+1・重複 no-op」を含む。
# **issue #111 の `Depends-on: #N` → 台帳 `dependsOn` 変換**も同 fixture 群で固定する: body からの
# marker parse (D3・複数 distinct union・フェンス/引用除外) / 番号→step id 解決の 2-pass (D1・既存
# step と同一 batch 割当予定 id の統合) / 接地の least fixpoint による fail-closed skip (D8・self-loop /
# 同一 batch 循環 / 未解決 (D1-iii) / 推移 skip の 4 ケースを 1 条件で閉じ ghost id を構造排除) /
# stdout `skipped:[{number,reason}]` (D9・exit 0 維持・reason で skip トリガーを区別) / blast radius を
# skip 候補 1 件に限定 (D2) / body 欠損の後方互換。**ghost id を出さない**ことは (hh) 推移ケースで
# 直接 assert し、生成した dependsOn 付き step が validate-plan-progress.py の非存在 id/循環 reject を
# 踏まないことを別途 [2]/[4] の schema 検査体系が担保する (本節は enqueue 出力の形を fixture で lock)。
ENQUEUE="$ROOT/scripts/decide-enqueue-steps.py"

# $1=ラベル $2=入力 JSON $3=期待する出力 JSON (フル一致。キー順・空白を正規化して比較)。
# 期待 JSON が `skipped` キーを省略した場合は `[]` を既定にしてから比較する (issue #111・D9):
# 実出力は常に `{"enqueue":…, "skipped":…}` の 2 キーを返すため、依存 fail-closed skip が絡まない
# 既存ケースは `enqueue` だけ書けばよい (省略 = skip 無し期待)。skip を検証するケースは `skipped` を
# 明示する。省略時に `[]` を補うため、想定外の skip が出れば既存ケースでも mismatch で捕まる。
assert_enqueue() {
  local label="$1" json="$2" want="$3" out got wantc
  out="$(printf '%s' "$json" | python3 "$ENQUEUE")" \
    || fail "$label: decide-enqueue-steps の実行に失敗した"
  got="$(printf '%s' "$out" | python3 -c 'import json,sys; print(json.dumps(json.load(sys.stdin),sort_keys=True,ensure_ascii=False))')"
  wantc="$(printf '%s' "$want" | python3 -c 'import json,sys; d=json.load(sys.stdin); d.setdefault("skipped",[]); print(json.dumps(d,sort_keys=True,ensure_ascii=False))')"
  if [ "$got" != "$wantc" ]; then
    fail "$label: 出力が期待と一致しない (got: $got / want: $wantc)"
  fi
}
...
assert_enqueue "(w) 未知キー (dependsOn) 無視 -> enqueue (#111 の前方拡張点)" \
  ...
  '{"enqueue":[{"id":"1","issue":{"number":78,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}}]}'
# (x)(y) epic 除外は batch 全体・順序非依存 (round1 🟡6): 同一 number が epic + 非 epic の両形で現れる
# 退化入力でも、その number は enqueue しない (epic_numbers で batch 全体を巻き込んで弾く)。epic-first と
# 非epic-first の両順序を固定し「number 単位で包含側に倒れない」ことを lock する (fail-safe の防御)。
assert_enqueue "(x) 退化 [78 epic, 78 非epic] (epic-first) -> no-op (batch 全体で弾く)" \
  '{"candidates":[{"number":78,"isEpic":true},{"number":78,"isEpic":false}],"steps":[]}' \
  '{"enqueue":[]}'
assert_enqueue "(y) 退化 [78 非epic, 78 epic] (非epic-first) -> no-op (順序非依存)" \
  '{"candidates":[{"number":78,"isEpic":false},{"number":78,"isEpic":true}],"steps":[]}' \
  '{"enqueue":[]}'

# --- `Depends-on: #N` marker -> 台帳 `dependsOn` 変換 (issue #111) --------------------------------
# 候補 dict の `body` から `Depends-on: #N` を parse (D3) し、番号 -> step id 解決 (2-pass・D1) と
# 接地の least fixpoint による fail-closed skip (D8) を経て step 雛形へ `dependsOn` を付与する。
# **決定論部分 (parse + 番号->id 解決 + fail-closed skip + dependsOn 付与) を smoke で閉じ、body 取得
# (network) は手動確認** (D4/D7)。skip surface は stdout の `skipped:[{number,reason}]` (exit 0 維持・D9)。
# `#51` を id "5" で持たせ「複数依存の distinct union 順」と「番号->id 解決」を同時に検証する fixture。
STEP51='{"id":"5","issue":{"number":51,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}}'
# (aa) 既存 step 依存: `Depends-on: #50` -> 既存 step (#50=id"1") の id へ解決した dependsOn を付与
assert_enqueue "(aa) 既存 step 依存 -> dependsOn 付与" \
  '{"candidates":[{"number":100,"body":"Depends-on: #50"}],"steps":['"$STEP1"']}' \
  '{"enqueue":[{"id":"2","issue":{"number":100,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null},"dependsOn":["1"]}]}'
# (bb) 複数依存 (D3-ii): 1 行複数 `#N` (カンマ/空白区切り) + 複数 `Depends-on:` 行を distinct union し
#      出現順で番号->id 解決する (#50=id"1", #51=id"5" -> ["1","5"]・重複 #50/#51 は 1 回だけ)
assert_enqueue "(bb) 複数依存 distinct union -> 出現順 dependsOn" \
  '{"candidates":[{"number":100,"body":"Depends-on: #50, #51\nDepends-on: #51 #50"}],"steps":['"$STEP1"','"$STEP51"']}' \
  '{"enqueue":[{"id":"6","issue":{"number":100,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null},"dependsOn":["1","5"]}]}'
# (cc) 同一 batch 内依存 (D1 の 2-pass): A(#100) が同一 tick discover の B(#101) に依存。pass 1 で B へ
#      割当予定 id "3" が確定済みのため A.dependsOn=["3"] を決定論で解ける (前方参照は validator 許容)
assert_enqueue "(cc) 同一 batch 内依存 -> 割当予定 id へ解決 (2-pass)" \
  '{"candidates":[{"number":100,"body":"Depends-on: #101"},{"number":101}],"steps":['"$STEP1"']}' \
  '{"enqueue":[{"id":"2","issue":{"number":100,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null},"dependsOn":["3"]},{"id":"3","issue":{"number":101,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}}]}'
# (dd) フェンス+引用除外 (D3-i): コードフェンス内 `#999` と引用行 `#888` は拾わず、生の `Depends-on: #50`
#      だけを取る (規約説明・他 issue 引用の偶発マーカーを権威ある依存宣言にしない)
assert_enqueue "(dd) フェンス+引用除外 -> 生マーカーのみ dependsOn" \
  '{"candidates":[{"number":100,"body":"```\nDepends-on: #999\n```\n> Depends-on: #888\nDepends-on: #50"}],"steps":['"$STEP1"']}' \
  '{"enqueue":[{"id":"2","issue":{"number":100,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null},"dependsOn":["1"]}]}'
# (ee) 自己依存 (self-loop・#57): 変換器が検知して当該候補を skip + surface (dependsOn に出さない —
#      出すと validate-plan-progress.py が self-loop 循環で全台帳 reject する)
assert_enqueue "(ee) 自己依存 self-loop -> skip + surface" \
  '{"candidates":[{"number":100,"body":"Depends-on: #100"}],"steps":[]}' \
  '{"enqueue":[],"skipped":[{"number":100,"reason":"self-loop"}]}'
# (ff) 同一 batch 循環 A⇄B (#57): 双方 skip + surface (接地の base case が無く非接地・D8)。
#      dependsOn に相手を出さない (出すと validator が循環 reject する)
assert_enqueue "(ff) 同一 batch 循環 A⇄B -> 双方 skip + surface" \
  '{"candidates":[{"number":100,"body":"Depends-on: #101"},{"number":101,"body":"Depends-on: #100"}],"steps":[]}' \
  '{"enqueue":[],"skipped":[{"number":100,"reason":"cycle"},{"number":101,"reason":"cycle"}]}'
# (gg) 未解決依存 (D1-iii・直接): `#999` は既存 step でも batch 候補でも無い -> 当該候補のみ skip + surface。
#      未解決が typo なら surface 行で human が気付く (D2 の緩和の入口)
assert_enqueue "(gg) 未解決依存 (直接) -> 当該候補のみ skip + surface" \
  '{"candidates":[{"number":100,"body":"Depends-on: #999"}],"steps":[]}' \
  '{"enqueue":[],"skipped":[{"number":100,"reason":"unresolved-dependency"}]}'
# (hh) 推移 skip (D8): A(#100)->B(#101)、B は #999 未解決で skip -> A も連鎖 skip。**A が B の割当予定 id
#      (ghost id) を dependsOn に出さない**ことを固定する (単一レベル解決の穴を接地条件で塞ぐ・D8 の核心)。
#      A の reason=unresolved-transitive / B の reason=unresolved-dependency を区別して assert
assert_enqueue "(hh) 推移 skip A->B(skip) -> A 連鎖 skip・ghost id 不出力" \
  '{"candidates":[{"number":100,"body":"Depends-on: #101"},{"number":101,"body":"Depends-on: #999"}],"steps":[]}' \
  '{"enqueue":[],"skipped":[{"number":100,"reason":"unresolved-transitive"},{"number":101,"reason":"unresolved-dependency"}]}'
# (ii) skip は当該候補に封じ込む (D2): 同一 tick に grounded な A(依存なし) と skip される B(#999 未解決)
#      が混在しても A は通常どおり enqueue される (blast radius を skip 候補 1 件に限定)
assert_enqueue "(ii) 混在: grounded A は enqueue・skip B は当該のみ (blast radius 限定)" \
  '{"candidates":[{"number":100},{"number":101,"body":"Depends-on: #999"}],"steps":[]}' \
  '{"enqueue":[{"id":"1","issue":{"number":100,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}}],"skipped":[{"number":101,"reason":"unresolved-dependency"}]}'
# (jj) skip 候補も pass 1 で id を消費する (D1): 先頭の skip 候補 B(#101・id"1"消費) の後、grounded な
#      A(#100) は id"2" を取る (skip で id に gap が出るが validator は連続性を要求せず無害)
assert_enqueue "(jj) 先頭 skip 候補が id を消費 -> 後続 grounded は次番号 (gap 無害)" \
  '{"candidates":[{"number":101,"body":"Depends-on: #999"},{"number":100}],"steps":[]}' \
  '{"enqueue":[{"id":"2","issue":{"number":100,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}}],"skipped":[{"number":101,"reason":"unresolved-dependency"}]}'
# (kk) `body` 欠損/空/依存マーカー無しは「依存なし」= 現行挙動不変 (D4 後方互換・dependsOn キーを付けない)
assert_enqueue "(kk) body 空/マーカー無し -> dependsOn 無し (現行挙動不変)" \
  '{"candidates":[{"number":100,"body":""},{"number":101,"body":"本文に依存マーカーは無い。Part of #5 だけ。"}],"steps":[]}' \
  '{"enqueue":[{"id":"1","issue":{"number":100,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}},{"id":"2","issue":{"number":101,"status":"created issue","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}}]}'
echo "[16/17] decide-enqueue-steps 判定ケース OK (単一追加/冪等 no-op/batch 採番/batch 内重複/空入力/終端突合/起点 id/非数値 id 除外/P<n> 形式追随/epic 除外(batch 全体・順序非依存)/拡張 candidate 契約/未知キー無視/**Depends-on 変換: 既存依存・複数 distinct union・同一 batch 2-pass・フェンス引用除外・self-loop/循環/未解決/推移 skip surface・blast radius 限定・id 消費・後方互換**)"

# --- 依存変換 (issue #111) ↔ 選別 jq (issue #51) の end-to-end 結線 (DoD) -------------------------
# DoD:「`Depends-on: #N` 付き issue を discover 経由で enqueue すると、生成 step に依存先 step id の
# `dependsOn` が付き、選別 jq が依存未終端の間 eligible にしない」。#111 の enqueue 出力 (dependsOn) を
# #51 の選別 jq (上記 SELECT_IMPLEMENTER_JQ・commands/harness-orchestrate.md と同一) へ実際に通し、
# 依存ガードが効くことを固定する。enqueue 直後の step は `created issue` だが依存ガードが効くのは
# `ready for implementation` へ進んだ後なので、生成 step の status を進めてから選別 jq に通す
# (「依存が解けるまで実装配車しない」= 本 issue の Problem「依存順の自動制御が discover 経由 issue に
# 効かない」穴を塞いだことの end-to-end 確認)。
DEP_E2E_NONTERM_DEP='{"id":"1","issue":{"number":50,"status":"starting review","githubState":"open"},"pr":{"number":null,"status":null,"githubState":null}}'
# (1) 非終端 #50 に依存する #100 を enqueue -> 生成 step に dependsOn=["1"] が付くこと
DEP_E2E_ENQ="$(printf '%s' '{"candidates":[{"number":100,"body":"Depends-on: #50"}],"steps":['"$DEP_E2E_NONTERM_DEP"']}' | python3 "$ENQUEUE")"
printf '%s' "$DEP_E2E_ENQ" | python3 -c 'import json,sys; s=json.load(sys.stdin)["enqueue"][0]; assert s["dependsOn"]==["1"], s' \
  || fail "(DoD e2e) enqueue が依存先 step id の dependsOn=[\"1\"] を生成しない (got: $DEP_E2E_ENQ)"
# (2) 生成 step を ready for implementation へ進めて選別 jq へ: 依存先が非終端の間は eligible にならない
DEP_E2E_PLAN_NONTERM="$(python3 - "$DEP_E2E_ENQ" "$DEP_E2E_NONTERM_DEP" <<'PY'
import json, sys
enq = json.loads(sys.argv[1])["enqueue"][0]
enq["issue"]["status"] = "ready for implementation"  # discover 後に issue レビューを通過して進んだ想定
print(json.dumps({"steps": [json.loads(sys.argv[2]), enq]}))
PY
)"
DEP_E2E_GOT_NONTERM="$(printf '%s' "$DEP_E2E_PLAN_NONTERM" | jq -c "$SELECT_IMPLEMENTER_JQ")"
[ "$DEP_E2E_GOT_NONTERM" = "[]" ] \
  || fail "(DoD e2e) #111 生成の dependsOn が #51 依存ガードで効かず、依存未終端でも eligible になった (got: $DEP_E2E_GOT_NONTERM)"
# (3) 依存先 #50 を終端 (closed issue) にすると当該 step が eligible になる (ガードが恒偽=vacuous でない負の自己検証)
DEP_E2E_PLAN_TERM="$(python3 - "$DEP_E2E_ENQ" <<'PY'
import json, sys
enq = json.loads(sys.argv[1])["enqueue"][0]
enq["issue"]["status"] = "ready for implementation"
dep = {"id": "1", "issue": {"number": 50, "status": "closed issue", "githubState": "closed"},
       "pr": {"number": None, "status": None, "githubState": None}}
print(json.dumps({"steps": [dep, enq]}))
PY
)"
DEP_E2E_GOT_TERM="$(printf '%s' "$DEP_E2E_PLAN_TERM" | jq -c "$SELECT_IMPLEMENTER_JQ")"
[ "$DEP_E2E_GOT_TERM" = '[{"id":"2","issueNumber":100}]' ] \
  || fail "(DoD e2e) 依存先を終端にしても eligible にならない (ガードが恒偽・vacuous) (got: $DEP_E2E_GOT_TERM)"
echo "[16/17] 依存変換 ↔ 選別 jq end-to-end 結線 OK (DoD: #111 生成の dependsOn が #51 依存ガードで効き依存未終端は非 eligible / 依存終端後は eligible の負の自己検証)"
...
# 依存マーカー契約の入力検証 (issue #111)
# (v2) dict 要素の body が文字列でも null でもない (数値) -> exit 2 (known key の型検証・isEpic と対称)
enq_rc=0
printf '%s' '{"candidates":[{"number":78,"body":5}],"steps":[]}' | python3 "$ENQUEUE" >/dev/null 2>&1 || enq_rc=$?
[ "$enq_rc" -eq 2 ] || fail "(v2) dict body 非文字列で exit 2 を期待したが exit $enq_rc"
echo "[16/17] decide-enqueue-steps 不正入力 exit 2 境界 OK (拡張 candidate 契約の number/isEpic/body 検証を含む)"
