...
assert_enqueue "(y) 退化 [78 非epic, 78 epic] (非epic-first) -> no-op (順序非依存)" \
  ...
  '{"enqueue":[]}'
echo "[16/17] decide-enqueue-steps 判定ケース OK (単一追加/冪等 no-op/batch 採番/batch 内重複/空入力/終端突合/起点 id/非数値 id 除外/P<n> 形式追随/epic 除外(batch 全体・順序非依存)/拡張 candidate 契約/未知キー無視)"
