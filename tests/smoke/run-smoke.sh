...
# 15. すべて通れば "SMOKE OK" を出して exit 0
set -euo pipefail
...
echo "[3/16] evidence.test ($TEST_CMD) exit 0"
...
echo "[4/16] (xviii) dependsOn A⇄B 循環 (issue #57 round 2) -> non-zero + 期待文言"
...
echo "[4/16] (v) statusEnums 残存 -> non-zero + 期待文言"
...
echo "[4/16] (viii) literal-guard -> non-zero + 期待文言"
...
echo "[4/16] (ix) isDraft drift -> non-zero + 期待文言"
...
echo "[5/16] gh 実行失敗 -> non-zero + 実行エラー文言 (drift と区別)"
...
echo "[5/16] gh 途中失敗 -> 検出済み drift + gh 失敗エラーの両方を出力して exit 1"
...
echo "[5/16] (xv) gh null 出力 -> dict 形状エラー + 蓄積済み drift を出力して exit 1"
...
echo "[6/16] reaggregate-has-blocker 判定ケース OK (fail-closed 境界・混在 sources・複数 findings 集計を含む)"
...
echo "[7/16] evaluate-stop-condition 判定ケース OK (round 上限・trend・trend の and/or 判別・fail-open 境界・has_blocker 抑止・round<3 短絡・同時成立・round=3 境界・不正入力 exit 2)"
...
echo "[8/16] decision-table 行数ガード OK (assert_route ケース数 $ROUTE_CASES == DECISION_TABLE エントリ数 $TABLE_ENTRIES)"
...
echo "[8/16] decide-orchestrator-route 判定ケース OK (全 role×outcome 25 行を網羅 + 不正入力 exit 2 境界 + A1 観測必須 fail-closed 境界)"
...
[ -f "$ORCH" ] || fail "[8/16] harness-orchestrate.md が存在しない: $ORCH"
...
[ -n "$LABEL_TOKENS" ] || fail "[8/16] decide-orchestrator-route.py から label_action トークンを抽出できない"
...
[ -n "$LABEL_SECTION" ] || fail "[8/16] 「label_action の実行」節を抽出できない"
LABEL_TOKEN_COUNT=0
for tok in $LABEL_TOKENS; do
  grep -Fq "\`$tok\`" <<<"$LABEL_SECTION" \
    || fail "[8/16] label_action トークン '$tok' の執行レシピが「label_action の実行」節に無い(decision script が emit するのに執行部にレシピが欠落 = add-label 無音 drift のリスク・round2 🔴1)"
...
[ "$LABEL_TOKEN_COUNT" -eq 4 ] \
  || fail "[8/16] label_action 非 null トークン数が期待(4)と不一致: $LABEL_TOKEN_COUNT ($LABEL_TOKENS)"
echo "[8/16] label_action 執行レシピ presence 検査 OK (decision script が emit する 4 トークンすべてに「label_action の実行」節の執行レシピが存在・round2 🔴1)"
...
echo "[9/16] reconcile-dispatch-marker 判定ケース OK (eligible/clear/wait/redispatch/sink 全 action・境界・fail-closed・max_retries:0 即 sink (issue #71) を含む)"
...
echo "[9/16] reconcile-dispatch-marker 不正入力 exit 2 境界 OK (max_retries 非負検証を含む)"
...
echo "[9/16] 選別(jq) 実装役の dispatchMarker ガード OK (marker 残存 step (wait/redispatch 中) を候補から除外・後方互換: dependsOn 無しの step は従来どおり配車)"
...
echo "[9/16] 選別(jq) 実装役の dependsOn ガード OK (issue #51・全依存終端/一部未終端/空配列/キー欠損/存在しないid(fail-closed)/依存先discuss型の6境界 + DoD(ii-a) 同一tick 2件以上eligible)"
...
echo "[9/16] 選別(jq) 対応役 / pr reviewer の reviewLock ガード OK (issue #37・in-flight な step を候補から除外)"
...
echo "[9/16] 選別(jq) issue reviewer / issue review worker の issueReviewLock + githubState + cross-phase ガード OK (issue #88・round1 🔴3: close済み/number未確定 issue を除外・in-flight step を除外・round2 🟡3: PR 実在 step を二重 dispatch から除外)"
...
PY
echo "[9/16] ledger_write 適用 (i) lw=null + clear_marker=true -> marker 単独削除 OK"
...
PY
echo "[9/16] ledger_write 適用 (ii) lw 非 null + clear_marker=true -> ledger_write と marker 削除の原子適用 OK"
...
PY
echo "[9/16] ledger_write 適用 (iii) clear_marker 省略 -> marker 不変 OK"
...
echo "[9/16] ledger_write 適用 (iv) lw=null + clear_marker 省略 -> no-op OK"
...
PY
echo "[9/16] ledger_write 適用 (v) marker_field 省略 -> 既定 dispatchMarker のみ削除 OK (issue #37 後方互換)"
...
if [ -d "$ROOT/.harness" ]; then
  ...
  echo "[10/16] 複製一致検査 (kit checkout) OK (templates/ 全ファイルのカバーを含む)"
else
  echo "[10/16] 複製一致検査は skip (.harness/ が無い = kit checkout ではない)"
fi
...
if command -v shellcheck >/dev/null 2>&1; then
  echo "[11/16] 抽出 script 群 (${#EXTRACTED_SCRIPTS[@]} 件) bash -n + shellcheck OK"
else
...
assert_collision "(g) 単一候補・非空 files -> safe" \
  '[{"id":"A","files":["x.py"]}]' \
  '{"groups":[],"safe":["A"]}'
echo "[12/16] detect-dispatch-collision 判定ケース OK (衝突なし/全件衝突/部分衝突(推移閉包)/独立 2 組/fail-closed/候補 0 件/単一候補)"
...
echo "[12/16] detect-dispatch-collision 不正入力 exit 2 境界 OK"
...
echo "[12/16] select-dispatch-representatives 代表選出述語(issue #55 を #87 で抽出)OK (safe dispatch/占有者 inject 除外(負の自己検証)/占有者ゼロ min id 代表 1 件(tie-break 入力順非依存)/占有者 group から 0 件(負の自己検証)/fail-closed 単独は常に持ち越し(負の自己検証)/kind 非依存代表/複合 partition)"
...
echo "[12/16] select-dispatch-representatives kind 網羅ガード OK (VALID_KINDS == $EXPECTED_KINDS)"
...
echo "[12/16] select-dispatch-representatives 不正入力 exit 2 境界 OK"
...
for f in "${DISPATCH_FILES[@]}"; do
  [ -f "$f" ] || fail "[13/16] dispatch ファイルが存在しない: $f"
  block="$(awk '/★最重要★/{cap=1} cap{print} cap&&/^---$/{exit}' "$f")"
  [ -n "$block" ] || fail "[13/16] ★最重要★ ブロックを抽出できない(区切り '---' が無い?): $f"
  for line in "${CANONICAL_CORE[@]}"; do
    grep -Fq "$line" <<<"$block" \
      || fail "[13/16] 共通コア行が $f の冒頭ブロックに無い(脱落 / 文言 drift): $line"
  done
done
echo "[13/16] 共通コア presence 検査 OK (6 dispatch ファイル × 5 canonical 行・逐語部分一致・単一ソースは CANONICAL_CORE)"
...
if core_block_has_all "$NEG_TMP"; then
  fail "[13/16] 共通コア presence 検査が vacuous(canonical 行を 1 本抜いても pass した)"
fi
echo "[13/16] 共通コア presence 検査の負のケース OK (canonical 行を抜いたコピーは fail 判定)"
...
[ -f "$IR_DISPATCH" ] || fail "[14/16] dispatch ファイルが存在しない: $IR_DISPATCH"

# (a) 判定 spec 本体 presence
[ -f "$IR_SPEC" ] || fail "[14/16] (a) 判定 spec が存在しない: $IR_SPEC"
echo "[14/16] (a) issue reviewer 判定 spec presence OK (roles/issue-reviewer.md)"
...
dispatch_defaults_to_spec "$IR_DISPATCH" \
  || fail "[14/16] (b) dispatch→spec 参照整合が崩れている(既定が spec を参照しない / skill 起動が opt-in sentinel より前に混入 / sentinel 欠落): $IR_DISPATCH"
echo "[14/16] (b) dispatch→spec 参照整合 OK (既定=spec 参照・個人 skill 起動は opt-in sentinel 配下のみ・既定に非混入)"
...
if dispatch_defaults_to_spec "$IR_NEG_DEFAULT_SKILL"; then
  fail "[14/16] (b) 参照整合が vacuous(既定経路に skill 起動を注入しても pass した)"
fi
...
if dispatch_defaults_to_spec "$IR_NEG_NO_SENTINEL"; then
  fail "[14/16] (b) 参照整合が vacuous(opt-in sentinel を抜いても pass した)"
fi
echo "[14/16] (b) 参照整合の負のケース OK (既定に skill 注入 / sentinel 除去のいずれも fail 判定)"
...
spec_has_all_lenses "$IR_SPEC" \
  || fail "[14/16] (c) 8 観点(L1-L8)/ 3 ファミリー見出しのいずれかが判定 spec に無い: $IR_SPEC"
echo "[14/16] (c) 8 観点 / 3 ファミリー見出し presence OK (roles/issue-reviewer.md)"
...
if spec_has_all_lenses "$IR_NEG_SPEC"; then
  fail "[14/16] (c) 観点見出し presence 検査が vacuous(L5 見出しを抜いても pass した)"
fi
echo "[14/16] (c) 観点見出し presence の負のケース OK (L5 見出しを抜いたコピーは fail 判定)"
...
[ -f "$ORCH_MD" ] || fail "[15/16] コマンドファイルが存在しない: $ORCH_MD"
...
[ "$EXPECTED_COUNT" -eq 18 ] \
  || fail "[15/16] 期待トークン数が 18 でない ($EXPECTED_COUNT) — decision script の route==sink outcome が変化した? 凍結マニフェストと一緒に更新が必要"

FROZEN_TOKENS="$(extract_frozen_tokens "$ORCH_MD" | sort)"
EXPECTED_SORTED="$(printf '%s\n' "$EXPECTED_TOKENS" | sort)"
if [ "$FROZEN_TOKENS" != "$EXPECTED_SORTED" ]; then
  fail "[15/16] 凍結停止条件マニフェストのトークン集合が decision script の sink outcome + git-status-guard と一致しない(過不足あり)。diff (< 期待 / > 凍結ブロック):
$(diff <(printf '%s\n' "$EXPECTED_SORTED") <(printf '%s\n' "$FROZEN_TOKENS") || true)"
fi
echo "[15/16] (a) DoD-2 凍結ブロック ↔ decision script 整合ガード OK (18 トークン集合一致: sink 17 + git-status-guard 1)"
...
if [ "$NEG_TOKENS" = "$EXPECTED_SORTED" ]; then
  fail "[15/16] 整合ガードが vacuous(凍結ブロックのトークンを 1 本抜いても集合一致してしまう)"
fi
echo "[15/16] (a) DoD-2 整合ガードの負のケース OK (トークンを抜いたコピーは不一致)"
...
for v in RE_PR RE_ISSUE RE_BARE RE_LOOSE; do
  [ -n "${!v}" ] || fail "[15/16] MODE-DETECTION-MANIFEST から $v を抽出できない(マニフェスト形式が変わった?)"
done
...
assert_classify() {
  ...
  [ "$got" = "$2" ] || fail "[15/16] パース判定: [$1] -> got=$got / want=$2"
}
...
echo "[15/16] (b) DoD-3 パース単体 OK (strict 受理 / 現行自由文例の非誤分類 / near-miss 検出)"

# (c) DoD-1: verbatim 雛形の presence 検査(fixture lock)。determinism は literal 置換で構造的に担保。
assert_present() {
  grep -qF "$1" "$ORCH_MD" || fail "[15/16] 雛形の verbatim 断片が欠落 / drift: $1"
}
...
echo "[15/16] (c) DoD-1 雛形 verbatim presence 検査 OK (両モード雛形 / 13 文 <STOP> / merge 代行明示指示 / issue 相の停止条件注記)"

# --- 16. 完了 -----------------------------------------------------------------
echo "[16/16] 全アサーション通過"
echo "SMOKE OK"
