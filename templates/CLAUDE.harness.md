...
- **issue フェーズの単一 sink (`need for human review`・issue #88)**: PR フェーズの `need for human review` sink を issue フェーズへ対称に写す。前進不能な issue (停止条件到達 = round 上限 / blocker 傾向未改善、または主観的エスカレーション) は原因を問わず単一の失敗経路 `issue.status = "need for human review"` へ集約し、`need for human review` ラベルを **issue に** 付与して人間へ surface する。issue reviewer が進められる status は `ready for implementation` / `need for human review` まで (PR reviewer の `ready for merge` / `need for human review` と対称の終端手前状態)。**issue review worker が `ready for implementation` を立てるのは越権 (例外なし)** — worker の前進 outcome は `waiting for review` 固定で、`ready for implementation` は issue reviewer の判定 (`clean_pass`) 経由でのみ到達する (decision script が構造的に担保。doer ≠ judge)。ラベル解除だけでは再 dispatch されない #12 結線を保つため、sink は status を書いてから集約する (詳細な配線は `commands/harness-orchestrate.md`「失敗経路」節)。**非対称の正直な明記 (issue #88 / #93)**: 「issue フェーズ = PR フェーズと対称」の主張は **1 点 (evidence gate) で成立しない** — (i) issue フェーズには実行して落とせる証拠 (test) が構造的に無く **evidence gate 相当が無い** (有界化は停止条件機構 + issueReviewLock hang 検知が担い、「偽の前進」検知は issue reviewer の別セッション読取に一本化される = PR responder より弱い)。**(ii) issue reviewer の判定エンジンの可搬性は issue #93 で解消した** — 既定 `ISSUE_REVIEW_MODE=spec` は kit 同梱の判定 spec `roles/issue-reviewer.md` (8 観点 / 3 ファミリー rubric を parity 深さで inline) を Read して実行し、他 repo は個人 skill 無しで動く (PR 側の既定 `code-review` が #49 で可搬化されたのと同型)。個人 skill `reviewing-github-issues` は opt-in `ISSUE_REVIEW_MODE=skill` へ後退し (PR opt-in `multi-angle` と対称)、opt-in 指定で skill 不在なら kit 既定 spec へ fail-soft する。**残る受容コスト**: kit 版 spec と個人 skill の rubric は 2 系統になり、両者の drift は機械検知不可 (個人 skill は repo 外 `~/.claude/skills` にあり smoke から読めない) で **best-effort の手動同期** — これは (i) の evidence gate 欠如・`reports[]` best-effort と同じ流儀の正直な受容コスト (詳細は `roles/issue-reviewer.md`「kit 版 rubric と個人 skill の drift」節)。**PR を伴わない issue 単体の close 代行は本 kit のスコープ外** (『終端の記録と merge 代行』「スコープ外(意図的)」を踏襲)。
...
- **機械検証は「ローカル validator の実行結果を Statuses API で自己申告する」方式**で維持する (台帳が commit されないため、GitHub ホストの CI が committed な台帳を検証する方式は成立しない)。**この機械検証 (schema/drift) は、F案では台帳がローカル (非コミット) であり独立ランナー (旧 harness-gate.yml) から見えないため、状態遷移を書いた本人セッションによる自己申告に必然的に縮退する。これは main-push 禁止ポリシー (台帳ローカル化) が生む受容コストである。台帳に対する ③ (作る人 ≠ 判定する人) の実質は、ほぼ**別セッションで起動する PR reviewer が実際の diff/状態を初見で読むこと単独**に依存する — F案では状態遷移をローカルにのみ書きコミットしないため、台帳の git history は遷移の監査証跡にならない (この点は下記『終端の記録と merge 代行』節「台帳はローカルで git 履歴に残らない」とも整合する)。自己申告は独立検証の代替ではなく便宜シグナル (convenience signal) である**:
  ...
  - **`--drift` の呼出頻度増 (受容コスト + follow-up)**: 状態遷移のたびに `--drift` (実台帳で 1 回あたり O(N)・約 23 回の gh 呼出) が走るため、旧方式 (日次 1 回) より gh API 呼出頻度が大幅に増える。これは F案の受容コストとして許容する。**対象 PR の head SHA に紐づく step のみへ drift を絞る (full O(N) を避ける) 最適化は follow-up** とする (本 PR ではコード最適化はせず文書化のみ)。**⚠ この絞込は cosmetic ではなく観測される drift 検知網羅を変えるため、受容ラインを spec として明記する (issue #54)**: 絞込後に保証されるのは「その遷移で処理対象になっている PR の step が、その瞬間 drift 無しであること」だけになる。**それ以外の step の GitHub 側乖離は、当該 step 自身が次に遷移するまで検知されない** — これは上の「遷移が無い間の乖離は次遷移まで見逃されうる」見逃し範囲を、同一 tick 内の他 step へさらに広げる変更である。実装時はこの受容ライン (絞込後も「遷移対象 step の drift は必ず検知する」ことは保証する) を守る。なお `--drift` は gh 認証を要するネットワーク経路のため `tests/smoke/run-smoke.sh` の対象外 (network は smoke 非対象・手動確認) であり、この受容ラインは spec 上の契約として固定する (絞込のコード実装自体は引き続き follow-up)。
...
- **二重オーケストレーターの排他 (軸1 = A・規約明記。issue #110) の受容コストを正直に記録する**: 同一台帳に対して `/loop` 常設巡航と `/goal` 有期キャンペーンを並行起動すると、**単一 writer が 2 人になる**。F案は競合検知を持たないため、両モードの同時書込は後勝ちが無音で前の書込を消す (#81 Problem・実観測済み: 2026-07-13 に並行 subagent の台帳上書きで前の編集が無音消失した)。本 kit はこれを **軸1 = A (規約明記)** で塞ぐ — 「同一台帳に対して両モードは択一 (並行起動しない)」を `commands/harness-orchestrate.md`「loop での回し方」節に明記する。**ただし A は機械強制がゼロである**: 「運用規約で守れる」は楽観で、人間が巡航中に別セッションで `/goal` を開けば後勝ち無音上書きは実際に起きる。Phase 3 は background 巡航を増やす方向のため、「巡航を回しっぱなしにして別セッションで集中運転を開く」= **人間が忘れる窓はむしろ広がる**。**機械強制 (B = tick 排他ロック・#81) は #77 (無人トリガー) / #79 (heartbeat・stale 閾値) が入る Phase 4 で導入する** (#77 着手が A→B 移行の forcing function)。中間案 (巡航 loop の軽量マーカー検出) は採らない (Phase 4 で B に一本化し二重メンテを避ける)。これは上記「機械検証は自己申告に縮退する」「遷移が無い間の乖離は次遷移まで見逃されうる」と同じ流儀の、F案が生む正直な受容コストである。
...
- **kit で編集できる範囲の区別**: 本規約 (`.harness/CLAUDE.harness.md` + `templates/` 複製) が全ロール共通の正。kit 同梱の dispatch ファイル + orchestrator 代筆配線 (`commands/harness-orchestrate.md` が単一 writer として台帳へ代筆) を持つのは **pr reviewer (`roles/pr-reviewer.md` 判定 spec + `roles/pr-reviewer-dispatch.md`)・実装役 / 対応役 (`roles/developer-implementer.md` / `roles/developer-responder.md`)・issue reviewer (`roles/issue-reviewer.md` 判定 spec + `roles/issue-reviewer-dispatch.md`)・issue review worker (`roles/issue-review-worker.md`)** の 5 経路 (issue #52 Phase A で pr reviewer、issue #88 で issue 2 ロールを配線、issue #93 で issue reviewer の判定 spec を kit 同梱化)。**issue reviewer の判定エンジン本体も issue #93 で kit 同梱化した** — 既定 `ISSUE_REVIEW_MODE=spec` は kit 同梱 spec `roles/issue-reviewer.md` (8 観点 rubric を inline) を実行し、pr reviewer が `roles/pr-reviewer.md` 判定 spec を持つのと対称になった。個人 skill `reviewing-github-issues` への依存は opt-in `ISSUE_REVIEW_MODE=skill` へ後退し、既定は kit 完結で可搬。**ただし kit 版 spec ⇔ 個人 skill の rubric drift は機械検知不可・best-effort 手動同期** (個人 skill は repo 外にあり smoke から読めない・受容コストの正直な明記・`roles/issue-reviewer.md`「kit 版 rubric と個人 skill の drift」節参照)。**pr review worker だけは kit 未同梱**で、本コマンド経由ではなく個人 skill `working-triaged-pr-for-loop` から手動 / loop 起動される独立ロール — 本規約を読んで best-effort で手編集するのみで kit からは強制できない。**したがって orchestrator が単一 writer として自動代筆する経路は上記 5 経路**であり、pr review worker は orchestrator 未経由の best-effort を待つ (issue #88 round1 🟡6: 本 PR で issue 2 ロールを kit 同梱・配線したため「pr-reviewer.md だけが判定結果を持つ / issue reviewer・worker は kit 未同梱」の旧記述を実態へ更新)。
