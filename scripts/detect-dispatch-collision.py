...
- `safe` と `groups` の全 id の和集合は入力の全 id 集合と一致し、互いに素(全 id はちょうど 1 箇所に
  現れる)。本 script は grouping(連結成分分割)だけを担い、`groups` / `safe` から実際に「今 tick どの
  候補を dispatch するか」を決める**代表選出**(占有者ゼロ group から step id 昇順で 1 件・占有者が居る
  group から 0 件・fail-closed 単独候補は常に持ち越し)は**下流の `select-dispatch-representatives.py`**
  が決定論で行う(issue #55 の代表選出述語を issue #87 で prose から抽出。本 script は #55 で不変・
  代表選出も占有者概念も持たない)。呼出側(prose)は本 script の `{groups, safe}` に各候補の kind を
  添えてその下流判定器へ渡す。
