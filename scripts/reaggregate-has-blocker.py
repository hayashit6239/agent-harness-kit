...
使い方:
    ...
    stdout に判定結果 JSON を返す:
        {"has_blocker": bool, "blocker_count": n, "unknown_source_blockers": [...]}

規則 (手順 5.5 の prose と同一):
- 🔴 → blocker
- 🟡 かつ sources に arch 系 (`arch` / `reviewing-pr-architecture` を含む) または
  google 系 (`google` を含む) → blocker
- 🟡 かつ sources が code-review 系 (`code-review` を含む) のみ → 非 blocker
- 🟡 かつ sources がどの既知系にも一致しない → blocker (fail-closed)。
...
def is_arch(source):
    return "arch" in source or "reviewing-pr-architecture" in source
